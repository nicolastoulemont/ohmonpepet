...
const ExistingBookingErrorObject = {
  existingBookingError:
    'A booking already exiting between this user and operator at theses dates',
}
...
export const createBooking = mutationField('createBooking', {
  ...
  async resolve(
    ...
    try {
      ...
      if (operator.account?.user?.id === userId) {
        return CannotBookingHimSelfErrorObject
      }
      ...
      const { priceWithOutApplicationFee, applicationFeeAmount } =
...
export const confirmBooking = mutationField('confirmBooking', {
  ...
  async resolve(_, { input: { id, source } }, { pubsub, user }) {
    try {
      const booking = await prisma.booking.update({
        ...
        data: {
          ...
          ...(source === 'OPERATOR' && {
            operatorConfirmationDate: new Date(),
            lastUpdatedBy: user.operatorId,
          }),
        },
        include: {
          user: {
...
export const cancelOnGoingBooking = mutationField('cancelOnGoingBooking', {
  ...
  async resolve(_, { input: { id, source, canceledReason } }, ctx) {
    try {
      ...
      console.error(error)
      return {
        errors: [{ key: 'booking', message: "Couldn't cancel the booking" }],
      }
    }
  },
})
...
export const authorizePayment = mutationField('authorizePayment', {
  ...
  async resolve(_, { id }, { user: { userId } }) {
    try {
      ...
      if (!booking.operator) {
        return {
          invalidOperatorError: 'This booking operator does not exist anymore',
        }
      }
      ...
      const shouldUseSetupIntentApi = isAfter(
...
export const updateBookingPaymentStatus = mutationField(
      ...
      try {
        ...
        if (booking.stripePayment?.paymentIntentId && !paymentMethodId) {
          ...
          if (!stripePaymentIntent) {
            return {
              paymentProcessorError:
                'The provider could not find the payment intent',
            }
          }
