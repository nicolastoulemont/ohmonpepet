...
export const bookingAnimal = objectType({
  name: 'BookingAnimal',
  isTypeOf: (data) =>
    Boolean((data as any).specieOptionId && (data as any).bookingId),
  definition(t) {
    t.implements('Node')
...
export const booking = objectType({
  ...
  definition(t) {
    ...
    t.field('user', {
      type: 'User',
      resolve: async (b) =>
        await prisma.booking.findUnique({ where: { id: b.id } }).user(),
    })
    t.field('operator', {
    ...
    t.field('service', {
      type: 'ServiceOption',
      resolve: async (b) =>
        await prisma.booking.findUnique({ where: { id: b.id } }).service(),
    })
    t.field('payment', {
      type: 'StripePayment',
      resolve: async (b) =>
    ...
    t.list.field('claims', {
      type: 'Claim',
      resolve: async (b) =>
        await prisma.booking.findUnique({ where: { id: b.id } }).claims(),
    })
    t.list.field('reviews', {
      type: 'Review',
      resolve: async (b) =>
        await prisma.booking.findUnique({ where: { id: b.id } }).reviews(),
    })
    t.list.field('animals', {
      type: 'BookingAnimal',
      resolve: async (b) =>
        await prisma.booking.findUnique({ where: { id: b.id } }).animals(),
    })
  },
})
