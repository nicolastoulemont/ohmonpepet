...
import { useFindPetSittersQuery } from 'generated/graphql'
...
export function PetSitters() {
	const [chartInfos, setChartInfos] = useState([])
	const { t } = useI18n(dashboardJSON)
	const chartContainer = useRef(null)
	const [width] = useDimensions(chartContainer)
	const { data, loading } = useFindPetSittersQuery()

	useEffect(() => {
		if (!data) return
		const lookup =
			data?.findPetSitters &&
			withFormattedDate(data.findPetSitters.profiles).reduce((acc, item) => {
